...
logging.info("File path available on s3 under %s bucket and folder name is %s", bucket_name, prefix)
logging.info(f"File path available on s3 under {bucket_name} bucket and folder name is {prefix}")
...
#filter files with .csv in their names and absolute paths
if all_files:
    csv_files = []
    error_files = []
...
#Additional column needs to be taken care of
...
logger.info("****************Final Dataframe from source which will be processing**************")
final_df_to_process.show()

#Enrich the data from all dimension table
#also create a datamart for sales_team and their incentive, address and all
#another datamart fpr customer who bought how much each day of month
...
#sales_team table
logger.info("**************Loading sales_team table into sales_team_table************")
...
final_sales_team_data_mart_df = s3_customer_store_sales_df_join\
            .select("store_id","sales_person_id","sales_person_first_name","sales_person_last_name",
                    "store_manager_name","manager_id","is_manager","sales_person_address","sales_person_pincode","sales_date","total_cost",
                    expr("SUBSTRING(sales_date,1,7) as sales_month"))
logger.info("**************Final Data for sales team data mart**************")
final_sales_team_data_mart_df.show()
parquet_writer.dataframe_writer(final_sales_team_data_mart_df, config.customer_data_mart_local_file)
...
if correct_files:
    ...
    logger.error("***********There is some error in process inbetween**************")
    sys.exit()
